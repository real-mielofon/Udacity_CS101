...
def crawl_web(seed,max_depth):
    ...
    while tocrawl:
        page = tocrawl.pop()
        pagedeep = tocrawldeep.pop()
        if pagedeep <= max_depth and page not in crawled:
            union(tocrawl, tocrawldeep, get_all_links(get_page(page)), pagedeep+1)
            crawled.append(page)
    return crawled
