...
def involved(courses, person):
    ...
    for hexamester in courses:
        for class_ in courses[hexamester]:
            if (courses[hexamester][class_]['teacher'] == person)  or \
                    (('assistant' in courses[hexamester][class_]) and \
                      (courses[hexamester][class_]['assistant'] == person)):
                if hexamester in result:
                    result[hexamester].append(class_)
                else:
                    result[hexamester] =[class_]    
    return result
